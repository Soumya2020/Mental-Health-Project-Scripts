...
vlen=4
featurelist=['location_variance','time_spent_moving','total_distance','AMS','unique_locations','entropy','normalized_entropy','time_home']
        ...
        for lr in lrs:
            for patience in patiences:
                ...
                for epoch in epochs:
                    ...
                    link = 'vectors_Qv_vlen4_with_missingdata_predict_at_12week.csvsync.csv'
                    ...
                    for user in data[user_col].unique():
                        print("User=====",user)
                        train_data = dataup[dataup[user_col] != user]                        
                        ...
                        brits = BRITS(X_train.shape[1], X_train.shape[2],
                                      n_classes=2,
                                      rnn_hidden_size=rnn_size,
                        ...
                        history = brits.fit(train_set=dataset_for_training, val_set=dataset_for_validating)
                        #feature_importances = dropout_feature_importance(brits, dataset_for_testing, X_test, y_test)
                        
                        # print("Feature Importances:", feature_importances)
                        #all_feature_importances.append(feature_importances)
                        
                        predictions = brits.classify(dataset_for_testing)  # prediction on unseen test data
                        pre_y = []
                    ...
                    #==================================================
                    df = pd.read_csv('BRIT_result.csv')
                    #df = pd.DataFrame()
                    df_len = len(df)
                    ...
                    df.to_csv("BRIT_result.csv", index=False)                    
